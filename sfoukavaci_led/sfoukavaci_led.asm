...
#define LED_OUT	LATA,5	; LED Output
...
BUFFPOS				; rolling buffer position
FLAGS				; various flags

		endc

#define BuffFull	FLAGS,0	; Buffer is full

		cblock  RAMINI1
BUFFL:F		; buffer of measured values ; 0xA0 - 0xAF | 10100000 - 10101111
BUFFM:F		; buffer of measured values ; 0xB0 - 0xBF | 10110000 - 10111111
BUFFH:F		; buffer of measured values ; 0xC0 - 0xCF | 11000000 - 11001111 
		endc
...
ClearRAM:
	...
	bsf	LED_OUT			; turn LED on
	bcf		BuffFull	; empty buffer
	clrf	BUFFPOS
...
AddMiddle
	...
	btfss	BuffFull	; check, if we have enough values in buffer
	goto	SkipCompare

	; dest = old - ADC
	; instruction subwf is X = X - w
	; it mean, we have to prepare OLD to temp
	...
	movf	INDF0,w		; buffer for L 10100000 - 10101111
	movwf	LBYTE
	bsf		FSR0L,4		; buffer for M 10110000 - 10111111
	movf	INDF0,w
	movwf	MBYTE
	movlw	b'10110000' ^ b'11000000'	; 01110000
	xorwf	FSR0L,f		; buffer for H 11000000 - 11001111
	movf	INDF0,w
	movwf	HBYTE

	movf	ADDATL,w
	subwf	LBYTE,f		; LBYTE = LBYTE - ADDATL
	movf	ADDATM,w
	subwfb	MBYTE,f		; MBYTE = MBYTE - ADDATM
	movf	ADDATH,w
	subwfb	HBYTE,f		; HBYTE = HBYTE - ADDATH

	; check for offset >= 512 ( and < 65536, mean negative)
	movf	HBYTE,w		; H byte must be 0, otherwise we have negative result
	btfss	STATUS,Z
	goto	SkipCompare

	movf	MBYTE,w		; if M byte is >=2 (>=512) skip as noise
	;andlw	b'11111110'
	btfss	STATUS,Z
	goto	SkipCompare
	...
	movf	MBYTE,w		; M byte is 1 (256-511) = valid
	btfss	STATUS,Z
	goto	BigEnough

	movlw	.150		; measure value
	subwf	LBYTE,w
	btfss	STATUS,C
	goto	SkipCompare
	
BigEnough:
	; OK, we have big enough change here
	movlb 0
	bcf		LED_OUT		; turn off LED
	movlw	.100		; 100 * 20 ms = 2 sec off
	movwf	ADCNT
OffOutLoop:
	BANKSEL	PIR0
	bcf	PIR0, TMR0IF
OffInLoop:
	btfss	PIR0, TMR0IF	; wait for 20 ms timer
	goto	OffInLoop
	decfsz	ADCNT,f			; do it 100x
	goto	OffOutLoop
	movlb	0
	bsf		LED_OUT		; turn on LED again

	movlw	.50		; 50 * 20 ms = 1 sec on
	movwf	ADCNT
OnOutLoop:
	BANKSEL	PIR0
	bcf	PIR0, TMR0IF
OnInLoop:
	...
	bcf		BuffFull	; empty buffer
	clrf	BUFFPOS
	return				; measured value is invalid, do not update it
...
SkipCompare:
	...
	; update current value to buffer
	movf	ADDATL,w
	movwf	INDF0		; buffer for L 10100000 - 10101111
	bsf		FSR0L,4		; buffer for M 10110000 - 10111111
	movf	ADDATM,w
	movwf	INDF0
	movlw	b'10110000' ^ b'11000000'	; 01110000
	...
	incf	BUFFPOS,f
	btfss	BUFFPOS,4	; buffer is full, roll over
	return
	;goto	DisplayADC

	clrf	BUFFPOS
	bsf		BuffFull	; mark buffer as full
	return

DisplayADC:		; send ADC value to serial
	movlb	0
	movf	ADDATL,w
	movwf	LBYTE
	movf	ADDATM,w
...
DisplayADC2:		; send ADC value to serial
	call	bin2dec
...
;	http://piclist.com/techref/microchip/math/radix/b2bp-24b8d-ob.htm
